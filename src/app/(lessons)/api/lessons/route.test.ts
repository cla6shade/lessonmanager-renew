...
const extractIds = (lessons: Lesson[]) => lessons.map((lesson) => lesson.id).sort();
...
describe('GET /api/lessons', () => {
  ...
  describe('인증된 사용자', () => {
    ...
    it('teacherId 필터로 레슨을 조회할 수 있어야 함', async () => {
      // Arrange
      const filteredLessons = mockLessons.filter((lesson) => lesson.teacherId === 1);
      const request = new NextRequest(
    ...
    it('locationId 필터로 레슨을 조회할 수 있어야 함', async () => {
      // Arrange
      const filteredLessons = mockLessons.filter((lesson) => lesson.locationId === 0);
      const request = new NextRequest(
  ...
  describe('에러 처리', () => {
    ...
    it('데이터베이스 에러 발생 시 500 에러를 반환해야 함', async () => {
      // Arrange
      vi.spyOn(prisma.lesson, 'findMany').mockRejectedValue(new Error('Database error'));
