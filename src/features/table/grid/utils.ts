...
export function getWorkingTeachersOnDate(
  ...
  return teachers.filter((teacher) => {
    ...
    const workingTimeData = JSON.parse(teacher.workingTime.times) as WorkingTimeData;
    const dayTimes = workingTimeData[dayOfWeek as keyof WorkingTimeData];
    return dayTimes && dayTimes.length > 0 && teacher.location.id === selectedLocation.id;
  });
}

export function getTeacherWorkingHours(teacher: ExtendedTeacher, dayOfWeek: string): number[] {
